<template>
  <ul class="divide-y divide-gray-300">
    <li v-for="vacancy in vacancies" :key="vacancy.id" class="py-5">
      ...
      <div>
        <a class="text-2xl font-bold text-brand-primary hover:underline" :href="`https://careers.lifeplusworldwide.com/jobs/${vacancy.id}`" target="_blank">{{ vacancy.name.replace(' -', ',') }}</a>
      </div>
