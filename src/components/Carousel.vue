    ...
    >
      ...
      <template #addons>
        <div class="absolute max-w-[42rem] px-4 md:px-0 w-full mx-auto flex justify-between items-center inset-0">
          ...
          <button @click="carousel.prev()" class="transition-transform hover:scale-110 w-8 h-8 flex items-center justify-center rounded-full bg-brand-primary text-white">
            <ArrowLeftIcon class="w-4 h-4" />
          </button>
          <button @click="carousel.next()" class="transition-transform hover:scale-110 w-8 h-8 flex items-center justify-center rounded-full bg-brand-primary text-white">
